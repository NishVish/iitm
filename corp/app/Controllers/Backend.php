...
{   
    ...
    // 1. Fetch all SQL tickets
    // Ensure the table name is correct and ticket_type matches exactly 'SQL'
    $data['sql_tickets'] = $db->table('tickets')
                            ...
                            //   var_dump($data);
                            //   exit;
    // 2. Handle SQL query form
    // use is('post') for a more reliable check in CI4
    if ($this->request->is('post')) {
        
        $sql = $this->request->getPost('sql');
        $data['sql'] = $sql;

        // Clean up the query string (removes accidental spaces/newlines)
        $cleanSql = trim($sql);

        try {
            // Check if it's a SELECT query (case-insensitive)
            if (stripos($cleanSql, 'SELECT') === 0) {
                $query = $db->query($cleanSql);
                
                // If query is valid, get results
                if ($query) {
                    $data['results'] = $query->getResultArray();
                } else {
                    $data['error'] = "Invalid Query Execution.";
                }
            } else {
    ...
    // Remove these for production, keep for debugging
    // var_dump($data); 
    // exit;
