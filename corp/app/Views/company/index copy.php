...
foreach($companies as $c) {
    // Process the complex contacts into a clean string for a single cell
    $contactString = "";
    if (!empty($c['contacts'])) {
        // Cleaning the pipe-separated data for spreadsheet readability
        $contactString = str_replace('|', ' | ', trim($c['contacts']));
    }

    $spreadsheetData[] = [
        '<input type="radio" name="main_id" value="'.$c['company_id'].'" onclick="event.stopPropagation()">',
        '<input type="radio" name="compare_id" value="'.$c['company_id'].'" onclick="event.stopPropagation()">',
        $c['company_name'] . "\n(" . $c['category'] . ")",
        $c['city'] . "\n" . $c['state'],
        $contactString,
        $c['company_id'] // Hidden ID for row clicks
    ];
}
?>
...
document.addEventListener("DOMContentLoaded", () => {
    // Define the columns based on your existing table structure
    const myColumns = [
        { title: 'Main' },
        { title: 'Compare' },
        { title: 'Company Name' },
    ...
    // Initialize your existing class
    const sheet = new Spreadsheet('spreadsheet-ui', {
        columns: myColumns,
        data: myData
    });

    // Add your row-click redirection logic
    // Since your class builds the table, we delegate the click to the tbody
    const tableBody = document.querySelector('#spreadsheet-ui tbody');
    if (tableBody) {
        tableBody.addEventListener('click', (e) => {
            const tr = e.target.closest('tr');
            // Don't redirect if clicking an input or the index column
            if (tr && e.target.tagName !== 'INPUT' && !e.target.classList.contains('row-index')) {
                const rowIndex = tr.sectionRowIndex;
                const rowData = myData[rowIndex];
                const companyId = rowData[rowData.length - 1]; // Get ID from last hidden element
