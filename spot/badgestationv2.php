...
if (preg_match('/Mobile\s*:\s*(\d+)/', $qrcode, $matches)) {
    $mobile = $matches[1];

    // Create a form that posts to the same page
    echo '<form id="mobileForm" method="POST">';
    echo '<input type="hidden" name="mobile" value="' . htmlspecialchars($mobile) . '">';
    echo '</form>';
    echo '<script>document.getElementById("mobileForm").submit();</script>';
    exit; // Stop further execution, let the form submit
}


    // Search for QR code match in tradevisitor table
    $query = "
