   ...
   public function fetchiitmdate()
{
    ...
    $isVisible = function($element) {
        ...
        if (stripos(str_replace(' ', '', $style), 'display:none') !== false) return false;

        $hiddenClasses = ['hidden', 'd-none', 'elementor-hidden'];
        $classes = explode(' ', $element->getAttribute('class') ?? '');
        if (count(array_intersect($classes, $hiddenClasses)) > 0) return false;

        if (strtolower($element->getAttribute('aria-hidden') ?? '') === 'true') return false;
    ...
    try {
        ...
        foreach ($eventContainers as $col) {
            ...
            if (in_array(strtoupper($texts[0]), ["HOME", "ESCALATE YOUR BRAND VISIBILITY WITH \u{200b}"])) continue;

            $eventName = $texts[0];
            if (in_array($eventName, $seen)) continue;
            $seen[] = $eventName;
