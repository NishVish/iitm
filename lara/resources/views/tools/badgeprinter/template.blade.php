@php
    ...
    $url = url('gettheontect/' . $lastSegment . '/7909075195');
    ...
    <div style="display: flex; gap: 20px; align-items: flex-start;">
        ...
        <div id="printSection" style="flex: 1; border: 1px solid #ccc; padding-top:0px;">
            <div class="image-container">
                ...
                <div id="temp">
                    <strong id="nameBox" style="font-size: 26px; color: black; text-transform: uppercase;">
                        Loading...
                    </strong><br>

                    <span style="font-size: 24px; color: black; text-transform: uppercase;">
                        <span id="companyBox">Loading...</span><br>
                        <span id="mobileBox">Loading...</span>
                    </span>
                </div>

                <img id="badgeImage" src="trade.jpg" class="background-image">
        ...
        <script>
            function calculateFontSize(text) {
                ...
                if (len > 32) base = 22;
                else if (len > 26) base = 24;
                else if (len > 20) base = 26;
                else base = 28;
            ...
            /* ===============================
               LOAD BADGE (UPDATED ROUTE)
            ================================*/
            function loadBadge() {

                // same logic as your contact system
                let path = window.location.pathname.split("/").filter(Boolean);
                let id = path[path.length - 1]; // last segment

                let query = "7909075195"; // replace later with QR or input

                fetch(`/gettheontect/${id}/${query}`)
                    .then(res => res.json())
                    .then(data => {

                        let name = data.scanned.name.toUpperCase();
                        let company = data.scanned.company.toUpperCase();

                        let nameSize = calculateFontSize(name);
                        let companySize = calculateFontSize(company) - 2;

                        document.getElementById("nameBox").innerText = name;
                        document.getElementById("nameBox").style.fontSize = nameSize + "px";

                        document.getElementById("companyBox").innerText = company;
                        document.getElementById("companyBox").style.fontSize = companySize + "px";

                        document.getElementById("mobileBox").innerText = data.scanned.mobile;
        ...
        <div>
          ...
          <input type="number" id="leftInput" style="width: 60px; padding: 5px; border: 1px solid #ccc; border-radius: 4px;">
                    ...
                    <div
                        style="display: flex; flex-direction: column; align-items: center; gap: 10px; font-family: sans-serif;">
            ...
            <div style="display: flex; gap: 30px; margin-bottom: 10px;">
                ...
                <div>
                    ...
                    <label style="display: block;">
                        <input type="radio" name="opacityToggle" id="disableOpacity" onchange="toggleOpacity(false)"
                            checked>
                        Hide Image in Print
                    </label>
                </div>
