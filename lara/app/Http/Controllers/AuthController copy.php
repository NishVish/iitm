...
class AuthController extends Controller
{
    ...
    public function requestOtpx(Request $request, $input = null, $eventid = null)
    {
        // ✅ ONLY ONE SOURCE (your form input)
        $inputValue = $request->input('input');

        // 🔵 DEBUG
        \Log::info('OTP Request Input:', ['input' => $inputValue]);
        ...
        // 🔵 DEBUG TYPE
        \Log::info('Detected input type:', [
            'type' => $type,
            'mobile' => $mobile,
            'email' => $email
        ]);

        // 📌 Event ID (optional fallback)
        $eventId = $request->input('event_id') ?? $eventid;
        // Debug (optional)
        // dd($mobile, $email, $eventId);

        // Continue your OTP logic...
        ...
        // $contact = $data['contact'] = DB::table(concat)
