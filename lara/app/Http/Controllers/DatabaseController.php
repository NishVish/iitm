    ...
    public function mergeOnUpdate($mobile = null, $email = null)
    {
        ...
        try {
            ...
            $contacts = DB::table('contact')
                ->join('company_data', 'contact.company_id', '=', 'company_data.company_id')
                ->whereIn('contact.contact_id', $allContactIds)
                ->where('company_data.entry_type', 'main') // 🔒 enforce type here
                ->select('contact.*')
                ->get();

            $allCompanyIds = DB::table('company_data')
                ->whereIn('company_id', $contacts->pluck('company_id'))
