...
<div class="content">
    ...
    <div class="container" style="display:flex; gap:20px; flex-wrap:wrap;">
        ...
        <div class="box" style="flex:1; min-width:250px;">
            <h3><?= esc($company['company_id']) ?> | <?= esc($company['company_name']) ?> (<?= $label ?>) |
                <?= esc($company['city']) ?>, <?= esc($company['state']) ?></h3>
            ...
            <?php if (!empty($contacts)): ?>
                <ul class="contact-list">
                    <?php foreach ($contacts as $c): ?>
                        <li>
                            <strong><?= esc($c['name']) ?></strong> (<?= esc($c['designation']) ?>) | ID:
                            <?= esc($c['contact_id']) ?>
        ...
        ">&times;</span>
            ...
            <form method="post" action="<?= site_url('discussion/add') ?>">
                ...
                <textarea name="message" id="message" rows="5" required
                    style="width:100%; padding:6px; margin-bottom:10px;"></textarea><br>
