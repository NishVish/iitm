...
<script>
    ...
    fetch('<?= base_url('mobile/events/upcoming') ?>')
    .then(res => res.json())
    .then(event => {
        ...
        let imgPath = "https://via.placeholder.com/800x500?text=Layout+Coming+Soon"; // default
        if (event && event.name) {
            document.getElementById("main-card").style.display = "block";

            // Set Location & Venue
            // document.getElementById("event").innerText = event.name;
            document.getElementById("event-venue-text").innerText = event.venue_details;

            // Set Image if exists
            if (event.event_image_url) {
                imgPath = event.event_image_url;
            }
