...
<!-- 1. Clickable link (manual fallback) -->
<a id="contactLink" href="{{ $url }}">
  Open Contact
</a>

<div id="contactBox">Loading...</div>
...
@php
    ...
    $id = $segments[count($segments) - 1] ?? 'default';
    $query = '7909075195';

    $url = url("gettheontect/$id/$query");
@endphp
...
function loadContact() {
    ...
    const url = document.getElementById("contactLink").href;
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            document.getElementById("contactBox").innerHTML = `
                <b>Name:</b> ${data.scanned.name} <br>
                <b>Company:</b> ${data.scanned.company} <br>
                <b>Mobile:</b> ${data.scanned.mobile} <br>
                <b>Email:</b> ${data.scanned.email}
            `;
