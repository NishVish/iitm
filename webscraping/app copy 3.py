...
def scrape_all_background():
    ...
    all_orgs = []
    seen_ids = set()  # To prevent duplicates across overlapping search terms
    page_size = 500   # Max allowed stable block size

    # Loop through the alphabet to force the server to reveal records beyond the 500 cap
    for current_char in string.ascii_lowercase:
        ...
        while True:
            ...
            payload = {
                ...
                "order": "lexic",
                "searchterm": current_char  # CRITICAL: Server requires a search constraint to paginate past 500
            }
            ...
            try:
                ...
                total_entities_for_letter = int(root.attrib.get("entities", 0))
                ...
                # Incremental Page Shifting
                start += page_size
                
                # Exit loop for this specific letter if we read all its entries
                if start >= total_entities_for_letter or len(orgs) < page_size:
                    break
        ...
        # Pause slightly between letters to avoid hitting anti-scraping blocks
        time.sleep(random.uniform(1.0, 2.0))

    # Save the deep combined database
